...
command="cd '$remote_directory' && tmuxinator start console -n '$hostname' -p .tmuxinator.yml -c node='$hostname' && sh --login"
echo $command
...
if [ $exit_status -ne 0 ]; then
  echo "Error: One or more commands failed to execute on $hostname. Please check the environment and configuration."
  exit $exit_status
fi